...
namespace проекция
{
    ...
    class Program
    {
            ...
            List<Door> listDoor = new List<Door>()//создаем список параметризированный Door и помещаем в него  запси
        {
            ...
            //List<string> doors = (from d in listDoor //перебираем все значения в списке дверей
            //                  select d.Manufacturer).ToList();//d в списке Доор,переменная d по очереди принимает все значения из списка,если условие выполнячется то добавляется в список
            ...
            List<string> doors = listDoor
                .Select(d=> d.Manufacturer)
                .Distinct()// метод улаляет повторенияудаляет повторения
                .ToList();
